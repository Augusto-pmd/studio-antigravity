...
export function FundTransferDialog({ profile, arsAccount, children }: { profile: UserProfile, arsAccount?: CashAccount, children: React.ReactNode }) {
  ...
  const handleSave = () => {
    ...
    if (!arsAccount) {
        toast({ variant: 'destructive', title: 'Error', description: `El usuario ${profile.fullName} no tiene una caja en ARS.` });
        return;
    }
    ...
    startTransition(async () => {
        try {
            ...
            const transactionRef = doc(collection(firestore, `users/${profile.id}/cashAccounts/${arsAccount.id}/transactions`));
            ...
            const accountRef = doc(firestore, `users/${profile.id}/cashAccounts/${arsAccount.id}`);
            const newBalance = arsAccount.balance + transferAmount;
            updateDocumentNonBlocking(accountRef, { balance: newBalance });

            toast({ title: 'Transferencia Realizada', description: `Se acreditaron ${formatCurrency(transferAmount, 'ARS')} a la caja de ${profile.fullName}.` });
            resetForm();
            setOpen(false);
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            {isSelf ? "Realice un ingreso o refuerzo a su caja personal en ARS." : "Realice un ingreso o refuerzo a la caja del usuario en ARS."}
          </DialogDescription>
        </DialogHeader>
