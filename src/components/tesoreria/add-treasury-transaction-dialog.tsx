...
export function AddTreasuryTransactionDialog({
  account,
  children,
}: {
  ...
  const handleSave = () => {
    ...
    if (type === 'Egreso' && transactionAmount > account.balance) {
      return toast({
        variant: 'destructive',
        title: 'Saldo insuficiente en la cuenta.',
      });
    }
    ...
    startTransition(() => {
      ...
      const transactionRef = doc(
        collection(firestore, `treasuryAccounts/${account.id}/transactions`)
      );
      ...
      batch
        ...
        .then(() => {
          toast({
            ...
            description:
              'El movimiento ha sido guardado y el saldo actualizado.',
          });
          resetForm();
          setOpen(false);
        })
        .catch((error) => {
          ...
          toast({
            ...
            description:
              'No se pudo registrar el movimiento. Es posible que no tengas permisos.',
          });
        });
    });
  };
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Registre un nuevo ingreso o egreso en esta cuenta.
          </DialogDescription>
        </DialogHeader>
        <div className="grid gap-4 py-4">
          <div className="space-y-2">
            ...
            <RadioGroup
              value={type}
              onValueChange={(v: 'Ingreso' | 'Egreso') => setType(v)}
