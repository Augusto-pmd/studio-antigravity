  ...
  const handleSave = async () => {
    if (!firestore || !user) {
      toast({
        variant: 'destructive',
        title: 'Error',
        description: 'No está autenticado o hay un problema de conexión.',
      });
      return;
    }
    if (!category || !amount || !exchangeRate || !date) {
      toast({
        ...
        description:
          'Categoría, Monto, Tipo de Cambio y Fecha son obligatorios.',
      });
      return;
    }
    ...
    try {
      ...
      toast({
        ...
        description:
          'Tu solicitud de fondos ha sido registrada y está pendiente de aprobación.',
      });
      resetForm();
      setOpen(false);
    } catch (error) {
      ...
      toast({
        ...
        description:
          'No se pudo enviar la solicitud. Es posible que no tengas permisos.',
      });
    } finally {
      setIsPending(false);
    }
